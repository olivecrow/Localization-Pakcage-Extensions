...
namespace LocalizationPackageExtensionsEditor
{
    ...
    public abstract class TranslationWindowBase : EditorWindow
    {
        ...
        protected abstract string path_icon { get; }

        public TargetCell targetCell;

        Locale GetLocale(string localeName) => allLocale.FirstOrDefault(x => x.LocaleName == localeName);
        ...
        protected virtual void OnEnable()
        {
            ...
            // baseTable = LocalizationSettings.StringDatabase.GetTable(selectedTableCollection.TableCollectionName, baseLocale);
        ...
        void CreateGUI()
        {
            ...
            var icon = new Image
            {
                image = AssetDatabase.LoadAssetAtPath<Texture2D>(path_icon),
                style =
                {
                    height = 20,
                    width = 20
        ...
        protected abstract void CreateSpecificGUI();
        protected abstract void RequestTranslation(Locale targetLocale, string text, Action<bool, string> result);
